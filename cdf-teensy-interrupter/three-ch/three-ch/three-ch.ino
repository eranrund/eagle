...
public:
  ...
  void (*m_done_off_period_cb)(void);
  void (*m_done_on_period_cb)(void);
  
  SoftPWM(int pin, void (*done_off_cb)(), void (*done_on_cb)()) {
    m_pin = pin;
    m_done_off_period_cb = done_off_cb;
    m_done_on_period_cb = done_on_cb;
  }
  ...
  void start(int pulse_len, int on_len) {
    if (m_active) {
        stop();
    }
  ...
  void done_off_period()
  {
    // Start timer to kill the output and then turn it on
    m_timer.begin(this->m_done_on_period_cb, m_on_len);
    digitalWriteFast(m_pin, HIGH);
  }
  ...
  void done_on_period()
  {
    digitalWriteFast(m_pin, LOW);
    m_timer.end();
    m_timer.begin(this->m_done_off_period_cb, m_pulse_len - m_on_len);
  }
};


void out1_on_helper(void);
void out1_off_helper(void);
SoftPWM out1(21, out1_off_helper, out1_on_helper);
void out1_on_helper(void) { out1.done_on_period(); }
void out1_off_helper(void) { out1.done_off_period(); }

void out2_on_helper(void);
void out2_off_helper(void);
SoftPWM out2(22, out2_off_helper, out2_on_helper);
void out2_on_helper(void) { out2.done_on_period(); }
void out2_off_helper(void) { out2.done_off_period(); }

void out3_on_helper(void);
void out3_off_helper(void);
SoftPWM out3(23, out3_off_helper, out3_on_helper);
void out3_on_helper(void) { out3.done_on_period(); }
void out3_off_helper(void) { out3.done_off_period(); }
...
void setup() {
 ...
 // out2.start(1000000, 500000);
 // out3.start(500000, 250000);
};
...
void loop() {
  ...
  pulse_duty_cycle_setpoint = map(analogRead(duty_cycle_pot), 0, 128, 10000, 100000);

  if ((interrupter_pulsewidth_setpoint != out1.m_on_len) ||
      (pulse_duty_cycle_setpoint != out1.m_pulse_len))
  {
    out1.start(pulse_duty_cycle_setpoint, interrupter_pulsewidth_setpoint);
  }
